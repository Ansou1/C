...
** Last update Sun Mar  9 15:47:46 2014 daguen_s
*/
...
int		display_by_sdl(char **map)
{
  ...
  SDL_Flip(all->screen);
  my_sleep();
  SDL_FreeSurface(all->wall);
  SDL_FreeSurface(all->ladder);
  SDL_FreeSurface(all->monster);
  SDL_FreeSurface(all->the_exit);
  SDL_FreeSurface(all->key);
  SDL_FreeSurface(all->the_start);
