/*
** ldi.c for ldi in /home/efilon/Projects/Cproject/Corewar
** 
** Made by sebastien antonico
** Login   <antoni_s@epitech.eu>
** 
** Started on  Sat Mar 30 13:10:29 2013 sebastien antonico
** Last update Sun Mar 31 00:38:59 2013 sebastien antonico
*/
...
int	read_first_oct_ldi(t_cw *cw, t_proc *p)
{
  int	v1;

  if (C_OCT1 == 1)
    {
      if (is_valid_nb(v1 = tab_to_int(READ_ON_VM(2), IND_SIZE) - 1, 1,
		      REG_NUMBER - 1))
...
int	read_second_oct_ldi(t_cw *cw, t_proc *p, int jumping)
{
  int	v2;

  if (C_OCT2 == 1)
    {
      if (is_valid_nb(v2 = tab_to_int(READ_ON_VM(2 + jumping), IND_SIZE) -
		      1, 1, REG_NUMBER - 1))
    ...
    v2 = tab_to_int(cw->vm + ((p->pc - cw->vm +
				...
				IDX_MOD)) % MEM_SIZE), IND_SIZE);
  v2 = tab_to_int(READ_ON_VM(v2 + jumping), IND_SIZE);
  return (v2);
}
...
void	read_ldi(t_cw *cw, t_proc *p)
{
  ...
  v1 = read_first_oct_ldi(cw, p);
  jumping = (C_OCT1 == 1) ? 1 : ((C_OCT1 == 2) ? DIR_SIZE : IND_SIZE);
  v2 = read_second_oct_ldi(cw, p, jumping);
  jumping += (C_OCT2 == 1) ? 1 : ((C_OCT2 == 2) ? DIR_SIZE : IND_SIZE);
  if (!is_valid_nb(v3 = tab_to_int(READ_ON_VM(2 + jumping), 1) - 1, 1,
		   REG_NUMBER - 1))
    v3 = 0;
  p->tmp_num_reg = v3;
  p->tmp_val = tab_to_int(READ_ON_VM((v1 + v2) % IDX_MOD), IND_SIZE);
}
